...
const typebotListener = async ({
  ...
  // getBodyMessage foi tipado pra WAMessage, então fazemos cast aqui
  const rawBody = getBodyMessage(msg as any);
  const body = (rawBody || "").toString();

  // Se não tiver texto útil, não segue pro Typebot
  if (!body.trim()) {
    return;
  }
